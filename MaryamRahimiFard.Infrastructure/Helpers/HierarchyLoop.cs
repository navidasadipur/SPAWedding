...
namespace MaryamRahimiFard.Infrastructure.Helpers
{
    public static class HierarchyLoop
    {
        ...
        public static string GetCourseCategoryHierarchy(CourseCategory entity, int? selectedItemParent = null, int? selectedItem = null)
        {
            selectedItemParent = selectedItemParent ?? 0;
            selectedItem = selectedItem ?? 0;
            var content = string.Empty;
            ...
            if (entity.Children.Any())
            {
                entity.Children.ToList().ForEach(item =>
                {
                    if (item.IsDeleted == false)
                    {
                        content += "<ul>" + GetCourseCategoryHierarchy(item, selectedItemParent, selectedItem) + "</ul>";
                    }
                });
            }
            content += "</li>";
        ...
        public static string GetCourseCategoryHierarchyForProducts(CourseCategory entity, int? selectedItemParent = null, int? selectedItem = null)
        {
            selectedItemParent = selectedItemParent ?? 0;
            selectedItem = selectedItem ?? 0;
            var content = string.Empty;
            if (entity.Id == selectedItemParent)
                content += "<li onclick=\"getCourseCategoryFeatures(" + entity.Id + ")\" data-jstree='{ \"selected\" : true,\"opened\": true }' id=\"pg_" + entity.Id + "\">" + entity.Title;
            else if (entity.Id == selectedItem)
                content += "<li onclick=\"getCourseCategoryFeatures(" + entity.Id + ")\" data-jstree='{ \"disabled\" : true }' id=\"pg_" + entity.Id + "\">" + entity.Title;
            else
                content += $"<li onclick=\"getCourseCategoryFeatures({entity.Id})\" id='pg_{entity.Id}'>{entity.Title}";
            ...
            if (entity.Children.Any())
            {
                entity.Children.ToList().ForEach(item =>
                {
                    if (item.IsDeleted == false)
                    {
                        content += "<ul>" + GetCourseCategoryHierarchyForProducts(item, selectedItemParent, selectedItem) + "</ul>";
                    }
                });
            }
            content += "</li>";
